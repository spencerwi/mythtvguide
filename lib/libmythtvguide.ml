...
let apply_channel_filter (channel_filter: string option) (guide: program_guide) =
    let filter_function = 
        ...
        | None -> (fun _ -> true)
        | Some s -> (fun chan -> Core.String.is_substring chan.channelName ~substring:s)
    in
...
let apply_program_name_filter (program_name_filter: string option) (guide: program_guide) =
    let program_filter = 
        ...
        | None -> (fun _ -> true)
        | Some s -> (fun prog -> Core.String.is_substring prog.title ~substring:s)
    in
    ...
    let filtered_channels =
        ...
        |> List.filter (fun (c: channel) -> (Core.List.is_empty c.programs) = false)
    in
        {guide with channels = filtered_channels}

let get_guide ?channel_filter:(channel_filter=None) ?program_name_filter:(program_name_filter=None) ?host:(host="127.0.0.1") (start_time: Core.Time.t) (end_time: Core.Time.t) :  [`Error of string | `Ok of program_guide ] Lwt.t  =
    let zone = Core.Lazy.force Core.Time.Zone.local in (* Don't adjust the time zone again *)
    let (start_str, end_str) = (
        Core.Time.format start_time "%Y-%m-%dT%H:%M:%S" ~zone,
        Core.Time.format end_time "%Y-%m-%dT%H:%M:%S" ~zone
    ) in
    let guide_url = Uri.of_string ("http://" ^ host ^ ":6544/Guide/GetProgramGuide?StartTime=" ^ start_str  ^ "&EndTime=" ^ end_str) in
    let headers = Cohttp.Header.of_list [("Accept", "application/json")] 
    in
    let%lwt (_, responseBody) = Client.get ~headers:headers guide_url in
        responseBody |> Cohttp_lwt__Body.to_string >|= fun stringBody -> 
            stringBody 
            |> (Core.String.filter ~f:Core.Char.is_print) 
